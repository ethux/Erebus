...
def _tokenize_mocked(text, entities, **kwargs):
    ...
    with patch("erebus.core.detect._predict_entities", side_effect=mock_predict):
...
def test_emails_tokenized_without_gliner_in_all_modes():
    text = (
        "Contacts: Sanne <sanne@voorbeeld-pg.test> and "
        "Daan <daan@voorbeeld-warehouse.test>"
    )
    for mode in MODES:
        sanitized, tokens = _tokenize_mocked(text, [], mode=mode)
        assert "sanne@voorbeeld-pg.test" not in sanitized, f"email leaked in {mode}"
        assert "daan@voorbeeld-warehouse.test" not in sanitized, f"email leaked in {mode}"
        assert len(tokens) == 2
        assert detokenize(sanitized, tokens) == text
    print("  ✓ Emails tokenized without GLiNER in all modes")
...
def test_detokenize_empty_map():
    ...
    text = "Nothing to detokenize Jansen here"
...
if __name__ == "__main__":
    tests = [
        ...
        test_emails_tokenized_without_gliner_in_all_modes,
        test_empty_string,
