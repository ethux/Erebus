...
def test_blacklist_phrase_with_regex_metacharacters():
    ...
    sanitized, _tokens = tokenize(text, blacklist=["Project.X (2024)"])
...
def test_blacklist_token_includes_kind():
    ...
    cases = [
        (f"Contact {email} today", email, "EMAIL"),
...
def _with_tmp_paths(fn):
    ...
    def wrapper():
        ...
        config._BLACKLIST_BLOCK_PATTERNS = patterns_before + [str(config.GLOBAL_BLACKLIST_PATH)]  # noqa: RUF005
        try:
            fn(tmp)
        finally:
...
def _run_cli(args, cwd: Path):
    ...
    from erebus.commands import blacklist as bl
    buf = io.StringIO()
    old_cwd = os.getcwd()
    os.chdir(cwd)
    try:
        with patch("sys.argv", ["erebus-blacklist"] + args), patch("sys.stdout", buf):  # noqa: RUF005
            rc = bl.main()
    finally:
        os.chdir(old_cwd)
