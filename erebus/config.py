...
def load_token_map(max_age_days: int = TOKEN_MAP_MAX_AGE_DAYS) -> dict:
    ...
    if isinstance(data, dict):
        try:
            mtime = datetime.fromtimestamp(TOKEN_MAP_PATH.stat().st_mtime, tz=UTC)
            if datetime.now(UTC) - mtime > timedelta(days=max_age_days):
                TOKEN_MAP_PATH.unlink(missing_ok=True)
                return {}
        except OSError:
            pass
    ...
    if TOKEN_MAP_PATH.exists():
        try:
            ...
            if isinstance(existing, dict) and "created_at" in existing:
                ...
                if datetime.now(UTC) - prev <= timedelta(days=max_age_days):
...
def _find_real_claude() -> str | None:
    """Find the real Claude binary (claude-real), not the shim."""
    env_override = os.environ.get("REAL_CLAUDE_BINARY")
...
def _find_real_codex() -> str | None:
    """Find the real Codex binary (codex-real), not the shim."""
    env_override = os.environ.get("REAL_CODEX_BINARY")
    if env_override and Path(env_override).exists():
        return env_override
    ...
    if GLOBAL_CONFIG_PATH.exists():
        try:
            with open(GLOBAL_CONFIG_PATH) as f:
                cfg = json.load(f)
            if cfg.get("real_codex_binary") and Path(cfg["real_codex_binary"]).exists():
                return cfg["real_codex_binary"]
        except (json.JSONDecodeError, KeyError):
            pass

    found = shutil.which("codex-real")
    if found:
        return found

    codex = shutil.which("codex")
    if codex:
        real = Path(codex).resolve().parent / "codex-real"
        if real.exists():
            return str(real)
    return None


def get_real_codex_binary() -> str:
    """Get the real Codex binary path, raising if not found."""
    result = _find_real_codex()
    if result is None:
        raise FileNotFoundError("Could not find codex-real binary. Run erebus-setup --editor codex first.")
    return result
...
class RepoConfig:
    ...
    # Optional durable known-value catalog, populated from approved local
    # source scans. Raw PII and credentials never belong in repo config.
    pii_catalog: CatalogConfig = field(default_factory=CatalogConfig)
    # Known-value DB (erebus/core/knownvalues.py) settings.
    known_values_scope: str = "global"  # global | project | hybrid
    known_values_retention: str = "days:7"  # permanent | days:N | session
    escape_window_minutes: int = 5
    unresolved_token_action: str = "warn"  # warn | block
...
def load_repo_config(cwd: str | None = None) -> RepoConfig:
    ...
    if config_path.exists():
        with open(config_path) as f:
            data = json.load(f)
        cfg_data = {k: v for k, v in data.items() if k in RepoConfig.__dataclass_fields__}
        catalog_data = cfg_data.get("pii_catalog")
        if isinstance(catalog_data, dict):
            cfg_data["pii_catalog"] = CatalogConfig(
                **{k: v for k, v in catalog_data.items()
                   if k in CatalogConfig.__dataclass_fields__}
            )
        elif not isinstance(catalog_data, CatalogConfig):
            cfg_data.pop("pii_catalog", None)
        cfg = RepoConfig(**cfg_data)
    else:
        cfg = RepoConfig()
