...
from erebus.filter import _parse_escapes, detokenize, tokenize
...
def test_angle_bracket_emails_detected_without_gliner():
    """Email addresses in contact syntax must not depend on NER."""
    from unittest.mock import patch

    text = (
        "Stakeholders: [PERSON_1_x] <sanne@voorbeeld-pg.test>, "
        "Daan <daan@voorbeeld-warehouse.test>"
    )
    with patch("erebus.core.detect._predict_entities", return_value=[]):
        sanitized, tokens = tokenize(text, mode="relaxed")
    assert "sanne@voorbeeld-pg.test" not in sanitized
    assert "daan@voorbeeld-warehouse.test" not in sanitized
    assert "<[EMAIL_ADDRESS_" in sanitized
    assert len(tokens) == 2
    print(f"  ✓ Angle-bracket emails tokenized: {sanitized}")
...
def test_custom_entity_detected():
    ...
    sanitized, _tokens = tokenize(text, extra_entities=["Acme BV"])
...
def test_clean_text_unchanged():
    ...
    _sanitized, tokens = tokenize(text)
    assert len(tokens) == 0
...
def test_escape_with_quotes():
    ...
    escaped, _cleaned = _parse_escapes('He said "Smith~" was there')
    assert "smith" in escaped
...
def test_escape_multiword_two():
    ...
    escaped, _cleaned = _parse_escapes("Ask John Smith~ about it")
    assert "smith" in escaped
...
def test_escape_multiword_four():
    ...
    escaped, _cleaned = _parse_escapes("Contact Jan Willem de Vries~ please")
...
def test_mode_relaxed_still_catches_secrets():
    ...
    sanitized, _tokens = tokenize(text, mode="relaxed")
    assert "sk-ant" not in sanitized
...
def test_mode_invalid_falls_back():
    ...
    text = "password: MySecret123!"
    sanitized, _tokens = tokenize(text, mode="invalid_mode")
    assert "MySecret123" not in sanitized
...
def test_custom_entities_always_tokenized():
    ...
    for mode in ("strict", "balanced", "relaxed"):
        ...
        sanitized, _tokens = tokenize(text, extra_entities=["Acme BV"], mode=mode)
        assert "Acme BV" not in sanitized
...
if __name__ == "__main__":
    tests = [
        # Original tests
        test_email_detected,
        test_angle_bracket_emails_detected_without_gliner,
